...
class Gaussian():
    def __init__(self, mu, precision):
        ...
        # --- 1) Symmetrize (cheap & required) ---
        precision = 0.5 * (precision + precision.transpose(-1, -2))

        # --- 2) Adaptive jitter to ensure PD ---
        eps = 1e-6
        I = torch.eye(precision.size(-1), device=precision.device, dtype=precision.dtype)
        jitter = eps

        for _ in range(8):     # fast, short loop
            try:
                cov = torch.inverse(precision + jitter * I)
                L = torch.linalg.cholesky(cov)
                break
            except RuntimeError:
                jitter *= 10      # increase jitter quickly
        else:
            raise RuntimeError("Could not stablize precision matrix even after jitter loop.")
